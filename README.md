...
1. **L4 Load Balancer (TCP/SSL Termination)**  
   - Размещён перед NGINX Ingress Controller.  
   - Выполняет распределение трафика по инстансам NGINX Ingress и терминирует TLS.  
   - Резервирование: **N+1** (один резервный L4-балансировщик на кластер).
...
3. **Внутренняя балансировка (Service Mesh / kube-proxy)**  
   - Используется `kube-proxy` + `iptables` или Cilium для распределения трафика между подами.  
   - Для критичных компонентов (Matching Engine) — sidecar-балансировка через Envoy/gRPC-LB.
...
> TLS-терминация выполняется на уровне NGINX Ingress Controller. Внешний L4-балансировщик (Cloudflare Load Balancer) используется только для распределения трафика между инстансами Ingress.
