...
Цель: при частичной деградации отдельных подсистем (trading, market data, KYC, withdrawals, analytics) сохранять работоспособность критического ядра биржи: **просмотр баланса, размещение/отмена ордеров, исполнение сделок, базовый вывод статуса операций**. Для этого для каждого сценария определены backend‑действия и ожидаемое поведение клиента (UX).

### Market Data Service (цены, стаканы) недоступен

**Бэкенд‑действия:**

- Перевести публичные Market Data API в **degraded read‑only mode**:  
  - отдавать последние зафиксированные `best_bid/best_ask` и `last_trade_price` из Aerospike (`market_price`) даже при задержке обновлений;  
  - отключить вспомогательные эндпоинты (глубокий стакан, агрегированные свечи низкого приоритета).
- Включить fallback:  
  - для экранов со списком инструментов использовать **агрессивный кэш** и краткосрочный TTL на `market_price`;  
  - временно ограничить частоту обновлений WebSocket‑стримов (throttling broadcast, batch‑обновления).
- Продолжить приём событий `priceupdate` в Redpanda, но:  
  - понизить приоритет потребителей, пишущих в ClickHouse (`price_history`), во избежание перегрузки OLAP‑кластера;  
  - включить backpressure и расширить очередь только в границах допустимого SLA.
- Для внутренних сервисов (risk/limits) использовать **last known good price** с явной отметкой о деградации.

**Клиент (UX):**

- В интерфейсе торговых экранов:  
  - показывать последний зафиксированный курс с плашкой «Данные рынка обновляются с задержкой»;  
  - частоту обновления сократить, но не блокировать обновления полностью.
- В стакане и ленте сделок:  
  - при полной недоступности данных показывать заглушку «Временно недоступно», но не блокировать выставление и отмену ордеров.  
- В мобильных/веб‑клиентах:  
  - не блокировать навигацию по рынкам и портфелю;  
  - явно отделять «реальную исполненную цену» (из истории сделок пользователя) от устаревшего UI‑курса.

### KYC / Verification Service недоступен

**Бэкенд‑действия:**

- Перевести KYC API в режим **очереди заявок**:  
  - продолжать принимать новые заявки и документы в PostgreSQL (`kyc_document_*`) и Spaces (`kyc-*-prod`), но помечать их статусом `pending_k yc_deferred`;  
  - временно остановить синхронные интеграции с внешними проверяющими провайдерами.
- Ввести жёсткий split по пользователям:  
  - для уже верифицированных пользователей KYC‑проверки кешировать и не дергать лишний раз;  
  - запретить изменение критичных PII‑полей (имя, адрес, документ) до восстановления сервиса.
- Для AML/комплаенса:  
  - сохранить события в Redpanda (`kyc_events`, `compliance_events`) и отложить тяжёлые проверки до восстановления;  
  - включить **priority queue** для событий с высоким риском (санкции, крупные депозиты).

**Клиент (UX):**

- При попытке прохождения/обновления KYC:  
  - показывать статус «Ваша заявка принята, проверка может занять больше времени обычного»;  
  - не падать ошибкой, а фиксировать черновик анкеты и загруженные документы локально до успешной отправки.
- Для уже верифицированных пользователей:  
  - не блокировать торговлю и выводы, пока нет явных рисков по AML.  

### Withdrawal / Fiat & Crypto Payout Service недоступен

**Бэкенд‑действия:**

- Перевести `withdrawal_request` в режим **приёма заявок без исполнения**:  
  - новые заявки записываются в PostgreSQL (`withdrawal_request_*`) со статусом `queued_due_to_maintenance`;  
  - фактические on‑chain/fiat‑проводки в сторонние платёжные шлюзы приостанавливаются.
- Балансы `wallet` и их блокировки:  
  - продолжать блокировать средства под вывод (hold) при создании заявки, чтобы не допустить double‑spend;  
  - удерживать локальный «журнал ожидания» для последующей синхронизации с провайдерами.
- Для обмена с банками/платёжными шлюзами:  
  - events о выводах сохранять в Redpanda/избыточные очереди и не отправлять в сторонние API до восстановления;  
  - запретить ретрай on‑chain транзакций, если нет уверенности в статусе (во избежание дублей).

**Клиент (UX):**

- Для экрана вывода:  
  - явно показывать баннер «Выводы временно обрабатываются с задержкой / поставлены в очередь»;  
  - разрешить создание заявки, но с чётким указанием, что списание и фактический перевод будут выполнены позже.
- В истории:  
  - заявки отображать в статусе «В очереди, будет обработано после восстановления сервиса», не скрывая их.  

### Analytics / Reporting (ClickHouse, отчётность) недоступен

**Бэкенд‑действия:**

- Перевести все тяжёлые аналитические запросы (BI‑дашборды, отчёты risk/compliance, выгрузки CSV) в режим **отложенной обработки**.  
- Продолжать писать события в Redpanda (`trading_events`, `audit_events`), но:  
  - остановить консьюмеры, пишущие в ClickHouse (`price_history`, `audit_log_*`), чтобы не терять данные при деградации;  
  - включить **WAL‑подобный архив** в S3/Spaces (например, `analytics-backlog`) для последующей реплеи в ClickHouse.  
- Для realtime‑риска/лимитов:  
  - использовать минимальные агрегаты из PostgreSQL/Aerospike (последние N часов), не полагаясь на OLAP.

**Клиент (UX):**

- Пользовательская аналитика (графики PnL, продвинутые отчёты):  
  - показывать упрощённые графики и базовые агрегаты за короткий период;  
  - явно помечать, что данные отчётов могут быть неполными.
- Админские/Backoffice‑панели:  
  - временно отключить тяжёлые отчёты и экспорты, оставить только критичные операции поддержки (freeze аккаунта, просмотр ключевых логов по пользователю по упрощённым фильтрам).

### Matching Engine / Trading Degradation (частичная деградация, не полный outage)

**Бэкенд‑действия:**

- Включить **circuit breaker** для горячих торговых пар:  
  - при росте latency/ошибок по конкретной паре временно ограничивать новые ордера `market`/`IOC`, разрешив только лимитные ордера с консервативными ограничениями;  
  - при критической нагрузке по паре перевести её в режим `post_only` (размещение лимитных ордеров без немедленного исполнения).
- **Load shedding**:  
  - приоритизировать отмену и исполнение уже существующих ордеров над размещением новых;  
  - замедлять низкоприоритетные операции (например, массовые отмены) через очереди.
- На уровне Redpanda и Aerospike:  
  - включить приоритизацию событий `trade_executed` и `wallet_update` над вторичными событиями (логирование, нотификации, вторичный кеш).

**Клиент (UX):**

- На экране торговли:  
  - явная плашка «Высокая нагрузка, исполнение ордеров может занимать больше времени»;  
  - возможное временное отключение/ограничение маркет‑ордеров по отдельным парам.  
- В WebSocket‑стримах:  
  - возможное снижение частоты обновления стакана и ленты сделок, но без полной остановки.  

### Degradation session/Redis (rate‑limit, кеш) недоступен

**Бэкенд‑действия:**

- При недоступности Redis:  
  - перевести часть rate‑limit проверок в **in‑process** счётчики с более жёсткими пределами;  
  - временно отключить не критичные кеши (например, вторичные кеши market‑data), чтобы не создавать лишнюю нагрузку на БД.
- При деградации Aerospike `session`:  
  - усиленно использовать JWT‑подписи и refresh‑токены без серверного состояния;  
  - временно сократить срок жизни сессий и частоту фонового продления.

**Клиент (UX):**

- При редких ошибках сессии:  
  - показывать «Сессия устарела, требуется повторный вход» вместо внутренних 5xx;  
  - не блокировать остальной функционал приложения.  
