...
| **DAU (Daily Active Users)** | ~4.3 млн | Оценка: 50% от MTU заходят ежедневно для торговли/мониторинга [1] |
...
|------------|----------------|----------------|-------------|-------------|
| **Пользовательские профили** | 120 млн | 8.5 MB | **~1.02 PB** | Документы, фото, метаданные |
| **История транзакций** | 86.4 млрд | 22 байта | **~1.9 TB** | Метаданные транзакций |
| **Ордера (ежемесячно)** | 2.7 млрд | 256 байт | **~691 GB/мес** → **~8.3 TB/год** | Архивация после исполнения |
| **Котировки в реальном времени** | 1.3 млрд/день | 32 байта | **~42 GB/день** → **~15.3 TB/год** | Изменения цен с минимальной задержкой |
| **Логи действий пользователей** | 189.5 млн/день | 512 байт | **~97.0 GB/день** → **~35.4 TB/год** | Аналитика, безопасность, compliance |
| **Метрики мониторинга** | ~500K samples/сек | 16 байт | **~20 TB/год** | VictoriaMetrics с 10x сжатием |
| **ИТОГО (активные + архивные данные)** | — | — | **~1.08 PB** | Основной объем — профили и KYC |
...
|-------------|------------------|--------|-------------|
| **UI/API запросы** | 12.8 Гбит/с | `(4.3M × 20 × 2 KB) / (86400 × 0.1)` | Пик в часы открытия рынков |
| **Торговые ордера** | 16.8 Гбит/с | `(4.3M × 24 × 0.5 KB) / (86400 × 0.1)` | Пик при волатильности рынка |
| **Поток цен WebSocket** | 5.1 Гбит/с | `250 × 1 обновление/сек × 64 байта × 4.3M DAU` | Рассылка всем подписчикам |
| **Репликация данных** | 3.0 Гбит/с | Оценка на основе ETL-процессов | Меж-DC синхронизация |
| **ИТОГО ПИКОВАЯ НАГРУЗКА** | **~37.7 Гбит/с** | — | Требуется инфраструктура с запасом 40+ Гбит/с |
...
|-------------|----------------|--------|
| UI/API запросы | 1660 ГБ | 4.3M × 20 × 2 KB |
| Торговые ордера | 2196 ГБ | 4.3M × 24 × 0.5 KB |
...
|-------------|-------------|-------------|--------|-------------|
| **Получение курсов / баланса** | 9,954 RPS | 99,540 RPS | `(4.3M × 20) / 86400` | Основная нагрузка на API-шлюзы |
| **Создание ордеров** | 11,952 RPS | 119,520 RPS | `(4.3M × 24) / 86400` | Нагрузка на matching engine |
| **Операции с фиатом** | 25 RPS | 250 RPS | `(4.3M × 0.05) / 86400` | Интеграция с банковскими API |
| **Криптовалютные переводы** | 15 RPS | 150 RPS | `(4.3M × 0.03) / 86400` | On-chain транзакции |
| **WebSocket подписки** | 107,500 RPS | 1,075,000 RPS | `250 × 4.3M DAU` | Частота рассылки сообщений |
...
- **Хранилище данных**: ~1.08 PB в год с доминированием KYC-данных
- **Критическая нагрузка**: 119K RPS на создание ордеров требует high-performance matching engine
- **WebSocket-трафик**: 1M+ сообщений/сек требует оптимизированной рассылки цен
...
|-------------|-----------|--------|--------|-------------|
| **Чтение курсов/балансов** | 99,540 | 69,678 | 24,885 | Ближайший ДЦ |
| **Создание ордеров** | 119,520 | 119,520 | 0 | Только `nyc3` |
| **WebSocket цены** | 1,075,000 | 752,500 | 268,750 | Anycast, ближайший ДЦ |
...
- Серверы в `nyc3` и `ams3` синхронизируют данные через Kafka
...
- **Трансляция**: Kafka → Aerospike в обоих ДЦ
...
- Требуемая емкость: `47,086 / 56,700 = 83%` от capacity одного инстанса

**Расчет для `ams3`**:
- HTTPS RPS: `269,060 × 0.25 = 67,265 RPS` 
- Новые TLS-соединения: `67,265 × 0.25 = 16,816 TPS`
- Требуемая емкость: `16,816 / 56,700 = 30%` от capacity одного инстанса

#### Сетевая пропускная способность

**Пиковая нагрузка**:
- Глобальная: **37.7 Гбит/с** (из раздела 2)
- `nyc3`: `37.7 × 0.7 = 26.4 Гбит/с`
- `ams3`: `37.7 × 0.25 = 9.4 Гбит/с`

**NGINX производительность сети** [5]:
- 16+ CPU: **70-72 Гбит/с** на 2×40G интерфейсах
- Запас пропускной способности: 2.6x для `nyc3`, 7.6x для `ams3`
...
| wallet | Балансы по валютам | 30 млрд | ~0.3 KB | ~9 TB | 120K / 120K | **Strong** | Шардирование по `(user_id, currency_code)`. Данные хранятся в Home Region пользователя. Кэш в Redis. |
| order | Активные и завершённые ордера | 2.7 млрд/мес | ~0.25 KB | ~691 GB/мес | 120K / 120K | **Strong** | Шардирование по валютной паре. Все шарды расположены в основном ДЦ (`nyc3`). |
| trade | Исполненные сделки | 2.7 млрд/мес | ~0.3 KB | ~830 GB/мес | 5K / 120K | **Strong** | Шардирование по валютной паре. Все шарды расположены в основном ДЦ (`nyc3`). Партиционирование по дате. |
| transaction | Все транзакции (депозиты, выводы, комиссии) | 86.4 млрд | ~0.2 KB | ~1.9 TB | 50 / 12K | **Strong** | Шардирование по `user_id`. Данные хранятся в Home Region пользователя. |
...
| price_history | История цен (1 обновление/сек на пару) | 1.3 млрд/день | ~0.05 KB | ~15.3 TB/год | 1K / 1.3M | **Best-effort** | Партиционирование по дате; реплицируется для аналитики; архив в cold storage. |
| audit_log | Логи действий (compliance, безопасность) | 189.5 млн/день | ~0.5 KB | ~35.4 TB/год | 10 / 200K | **Best-effort** | Централизованная запись через Kafka → ClickHouse в основном ДЦ, репликация для аналитики. |
...
flowchart TD
    subgraph nyc3["DigitalOcean nyc3 (США) - Основной ДЦ"]
        ...
        C["Aerospike Cluster: market_price, order_book"]
        D["DigitalOcean Spaces: KYC-бинарники (US)"]
        E["PostgreSQL: currency, bank_account (US)"]
        F["Redis Cluster: session, rate_limits, cache"]
    end
    ...
    subgraph ams3["DigitalOcean ams3 (ЕС) - Резервный ДЦ"]
        ...
        I["Aerospike Cluster: реплики market_price"]
        J["DigitalOcean Spaces: KYC-бинарники (EU)"]
        K["PostgreSQL: реплики currency, bank_account (EU)"]
        L["Redis Cluster: реплики session, cache"]
    end
...
- обработки пиковых нагрузок до **120K QPS** на запись
...
| **Envoy Proxy** | L4 Load Balancer, Service Mesh, динамическая маршрутизация | Работает как L4-прокси (2 инстанса: 1 активный + 1 резервный), распределяя TLS-трафик от Cloudflare между инстансами NGINX Ingress. Обеспечивает отказоустойчивость при пиковой сетевой нагрузке **~37.7 Гбит/с**. Интегрируется с service mesh для gRPC между Matching Engine и Wallet, поддерживает динамическую маршрутизацию для Adaptive Order Book Partitioning. |
...
| **Cilium + eBPF** | Сетевая маршрутизация внутри Kubernetes-кластера | Заменяет `kube-proxy` и `iptables`, обеспечивая **O(1)-сложность маршрутизации**, L7-наблюдаемость и безопасность на уровне ядра Linux. Минимизирует задержку между сервисами, что критично при пиковой нагрузке > 100K RPS и требованиях к latency < 1 мс для исполнения ордеров. |
...
| **Liquibook** | Ядро Matching Engine | Открытая C++ библиотека для построения order matching engine с производительностью до **2.5 млн операций/сек**. Позволяет реализовать core-логику исполнения ордеров с поддержкой stop-loss, IOC, AON и depth book. Интегрируется с Adaptive Order Book Partitioning для динамического масштабирования. |
...
| **CI/CD и деплой** | **Blue/Green через Argo Rollouts**, автоматический откат при >1% ошибок | Canary-деплой: сначала 5% трафика → анализ метрик (latency, error rate) → полный rollout. Rollback — за <10 сек. |
