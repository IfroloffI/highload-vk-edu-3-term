...
| **История транзакций** | 86.4 млрд | 50 байт | **~4.3 TB** | Метаданные транзакций |
| **Ордера (ежемесячно)** | 2.7 млрд | 256 байт | **~691 GB/мес** → **~8.3 TB/год** | Архивация после исполнения |
| **Котировки в реальном времени** | 1.3 млрд/день | 100 байт | **~130 GB/день** → **~47.5 TB/год** | Изменения цен с минимальной задержкой |
...
|-------------|------------------|--------|-------------|
| **UI/API запросы** | 163 Мбит/с | `(4.35M × 20 × 16,384 бит) / (86400 × 0.1)` | Пик в часы открытия рынков |
| **Торговые ордера** | 49 Мбит/с | `(4.35M × 24 × 4,096 бит) / (86400 × 0.1)` | Пик при волатильности рынка |
| **Поток цен WebSocket** | 8.7 Гбит/с | `4.35M × 15 подписок × 100 байт × 8 бит / 0.1` | Рассылка по 15 парам на пользователя |
| **Репликация данных** | 3.0 Гбит/с | Оценка на основе ETL-процессов | Меж-DC синхронизация |
| **ИТОГО ПИКОВАЯ НАГРУЗКА** | **~12.8 Гбит/с** | — | Требуется инфраструктура с запасом 15+ Гбит/с |
...
| **WebSocket подписки** | 65,250,000 RPS | 652,500,000 RPS | `4.35M × 15` | Сообщения для всех подписчиков |
| **Аутентификация** | 5,000 RPS | 50,000 RPS | Оценка на основе DAU | Нагрузка на сервис аутентификации |

**Ключевые выводы по нагрузке**:
- **Пиковая сетевая нагрузка**: ~13 Гбит/с требует 15+ Гбит/с инфраструктуры
- **Хранилище данных**: ~1.08 PB в год с доминированием KYC-данных
- **Критическая нагрузка**: 121K RPS на создание ордеров требует high-performance matching engine
- **WebSocket-трафик**: 65M+ сообщений/сек требует оптимизированной рассылки цен
